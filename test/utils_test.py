...
if __name__=="__main__":
    dataset=np.random.randint(-1, 24, size=(batch_size*2, sequence_aa))
    inputs, targets, labels=prepareBatch(dataset)
    ...
    #Check labels
    print('>>LABELS: They should be one hot vecs:', labels[0, 0, 0])
    print('>>LABELS: They should be one hot vecs:', labels[0, 0, 1])
    assert np.sum(labels)==(batch_size*sequence_length*num_predic_terms)
    print('\n\n')

    #Check target
    b,t, t_pred=[0,0,0]
    real_target_1=np.reshape(inputs[b, t+1], -1)
    targets_1=np.reshape(targets[b,t, t_pred][np.argmax(labels[b,t, t_pred])], -1)
    print(real_target_1)
    print(targets_1)
